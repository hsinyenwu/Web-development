...
Note: here we want container inside the navbar, so only the items are centered. We don't want the entired navbar centered.
```html
<!doctype html>
<html>
<head>
...
Now when we shrink the window size to mobile, the items except the brand disappear and there is no little hamburger. 
```html
<!doctype html>
<html>
<head>
...
The \<span class="icon-bar">\</span> is only for making bars in the hamburger button.  
